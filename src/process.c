...
search_path(const char *cmd)
{
  ...
  while (dir) {
    ...
    if (executable_exists(full_path)) { free(path_copy); return full_path; }
    free(full_path);
    dir = strtok(NULL, ":");
  }
  free(path_copy);
...
{
  if (strchr(cmd, '/')) {
    if (!executable_exists(cmd)) { errno = ENOENT; mrb_sys_fail(mrb, cmd); }
  } else {
    char *found = search_path(cmd);
    if (!found) { errno = ENOENT; mrb_sys_fail(mrb, cmd); }
    free(found);
  }
}
...
{
  if (src != dest) {
    if (dup2(src, dest) == -1) { mrb_sys_fail(mrb, "dup2 failed"); }
    close(src);
  }
}
...
{
  if (mrb_fixnum_p(val)) return mrb_fixnum(val);
...
mrb_f_spawn(mrb_state *mrb, mrb_value klass)
{
  ...
  if (argc > 0 && mrb_hash_p(argv[argc - 1])) { opts = argv[argc - 1]; argc--; }

  if (!mrb_nil_p(opts)) {
    mrb_value out_val, err_val, in_val;
    out_val = mrb_hash_get(mrb, opts, mrb_symbol_value(mrb_intern_lit(mrb, "out")));
    if (!mrb_nil_p(out_val)) { out_fd = fd_from_value(mrb, out_val); if (out_fd < 0) mrb_raise(mrb, E_ARGUMENT_ERROR, "bad out option"); }
    err_val = mrb_hash_get(mrb, opts, mrb_symbol_value(mrb_intern_lit(mrb, "err")));
    if (!mrb_nil_p(err_val)) { err_fd = fd_from_value(mrb, err_val); if (err_fd < 0) mrb_raise(mrb, E_ARGUMENT_ERROR, "bad err option"); }
    in_val = mrb_hash_get(mrb, opts, mrb_symbol_value(mrb_intern_lit(mrb, "in")));
    if (!mrb_nil_p(in_val)) { in_fd = fd_from_value(mrb, in_val); if (in_fd < 0) mrb_raise(mrb, E_ARGUMENT_ERROR, "bad in option"); }
  }

  if (mrb_array_p(argv[0])) {
    mrb_value *arr; mrb_int arr_len;
    arr = RARRAY_PTR(argv[0]); arr_len = RARRAY_LEN(argv[0]);
    if (arr_len < 1) mrb_raise(mrb, E_ARGUMENT_ERROR, "first element of array must be a command");
    prog = arr[0]; prog0 = (arr_len > 1) ? arr[1] : prog; start = 1;
    check_command(mrb, mrb_string_value_cstr(mrb, &prog));
  } else {
    prog = argv[0]; prog0 = argv[0];
    if (argc == 1) {
      pid = fork();
      if (pid == 0) {
      ...
      if (pid < 0) mrb_sys_fail(mrb, "fork failed");
  ...
  {
    const char **cargv = (const char **)mrb_alloca(mrb, sizeof(char *) * (argc - start + 2));
    cargv[0] = mrb_string_value_cstr(mrb, &prog0);
    for (i = start; i < argc; i++) cargv[i - start + 1] = mrb_string_value_cstr(mrb, &argv[i]);
...
mrb_f_kill(mrb_state *mrb, mrb_value klass)
{
  ...
  if (mrb_fixnum_p(sigo)) {
    ...
    if (mrb_string_p(sigo)) { name = RSTRING_PTR(sigo); namelen = (size_t)RSTRING_LEN(sigo); }
    else { name = mrb_sym2name_len(mrb, mrb_symbol(sigo), &namelen); }
    if (namelen >= 3 && strncmp(name, "SIG", 3) == 0) { name += 3; namelen -= 3; }
    for (i = 0; signals[i].name != NULL; i++) {
      symlen = strlen(signals[i].name);
      if (symlen == namelen && strncmp(name, signals[i].name, symlen) == 0) { signo = signals[i].no; break; }
    }
  ...
  sent = 0;
  if (kill(pid, signo) == -1) mrb_sys_fail(mrb, "kill");
  sent++;
  while (argc-- > 0) {
    if (!mrb_fixnum_p(*argv)) mrb_raisef(mrb, E_TYPE_ERROR, "wrong argument type %S (expected Fixnum)", mrb_obj_value(mrb_class(mrb, *argv)));
    if (kill(mrb_fixnum(*argv), signo) == -1) mrb_sys_fail(mrb, "kill");
    sent++; argv++;
  }
...
mrb_f_fork(mrb_state *mrb, mrb_value klass)
{
  ...
  case 0:
    ...
    if (!mrb_nil_p(b)) { mrb_yield_argv(mrb, b, 0, NULL); _exit(0); }
    return mrb_nil_value();
  case -1: mrb_sys_fail(mrb, "fork failed"); return mrb_nil_value();
  default: return mrb_fixnum_value(pid);
  }
}
...
retry:
  ...
  if (result < 0) { if (errno == EINTR) goto retry; return -1; }
  return result;
}
...
{
  mrb_int pid, flags = 0; int status;
  mrb_get_args(mrb, "i|i", &pid, &flags);
  if ((pid = mrb_waitpid(pid, flags, &status)) < 0) mrb_sys_fail(mrb, "waitpid failed");
  if (!pid && (flags & WNOHANG)) { mrb_gv_set(mrb, mrb_intern_lit(mrb, "$?"), mrb_nil_value()); return mrb_nil_value(); }
...
{
  mrb_int argc; mrb_value *argv; time_t beg, end;
  beg = time(0);
  mrb_get_args(mrb, "*", &argv, &argc);
  if (argc == 0) {
    sleep((32767<<16)+32767);
  } else if (argc == 1) {
    struct timeval tv; int n;
    if (mrb_fixnum_p(argv[0])) { tv.tv_sec = mrb_fixnum(argv[0]); tv.tv_usec = 0; }
    else { tv.tv_sec = mrb_float(argv[0]); tv.tv_usec = (mrb_float(argv[0]) - tv.tv_sec) * 1000000.0; }
    n = select(0, 0, 0, 0, &tv);
    if (n < 0) mrb_sys_fail(mrb, "mrb_f_sleep failed");
  } else { mrb_raise(mrb, E_ARGUMENT_ERROR, "wrong # of arguments"); }
  end = time(0) - beg;
...
{
  int ret; mrb_value *argv, pname; const char *path; mrb_int argc; void (*chfunc)(int);
  fflush(stdout); fflush(stderr);
  mrb_get_args(mrb, "*", &argv, &argc);
  if (argc == 0) mrb_raise(mrb, E_ARGUMENT_ERROR, "wrong number of arguments");
  pname = argv[0]; path = mrb_string_value_cstr(mrb, &pname);
  chfunc = signal(SIGCHLD, SIG_DFL); ret = system(path); signal(SIGCHLD, chfunc);
  if (WIFEXITED(ret) && WEXITSTATUS(ret) == 0) return mrb_true_value();
  return mrb_false_value();
}

mrb_value
mrb_f_exit(mrb_state *mrb, mrb_value klass) { return mrb_f_exit_common(mrb, 0); }
mrb_value
mrb_f_exit_bang(mrb_state *mrb, mrb_value klass) { return mrb_f_exit_common(mrb, 1); }

static mrb_value
mrb_f_exit_common(mrb_state *mrb, int bang)
{
  mrb_value status; int istatus, n;
  n = mrb_get_args(mrb, "|o", &status);
  if (n == 0) status = (bang) ? mrb_false_value() : mrb_true_value();
  if (mrb_type(status) == MRB_TT_TRUE) istatus = EXIT_SUCCESS;
  else if (mrb_type(status) == MRB_TT_FALSE) istatus = EXIT_FAILURE;
  else { status = mrb_convert_type(mrb, status, MRB_TT_FIXNUM, "Integer", "to_int"); istatus = mrb_fixnum(status); }
  if (bang) _exit(istatus); else exit(istatus);
}

mrb_value
mrb_f_pid(mrb_state *mrb, mrb_value klass) { return mrb_fixnum_value((mrb_int)getpid()); }
mrb_value
mrb_f_ppid(mrb_state *mrb, mrb_value klass) { return mrb_fixnum_value((mrb_int)getppid()); }
...
{
  int i = mrb_fixnum(mrb_iv_get(mrb, self, mrb_intern_lit(mrb, "@status")));
  if (WIFEXITED(i)) return mrb_fixnum_value(WEXITSTATUS(i));
  return mrb_nil_value();
}
...
{
  int i = mrb_fixnum(mrb_iv_get(mrb, self, mrb_intern_lit(mrb, "@status")));
  if (WIFSTOPPED(i)) return mrb_fixnum_value(WSTOPSIG(i));
  return mrb_nil_value();
}
...
{
  int i = mrb_fixnum(mrb_iv_get(mrb, self, mrb_intern_lit(mrb, "@status")));
  if (WIFSIGNALED(i)) return mrb_fixnum_value(WTERMSIG(i));
  return mrb_nil_value();
}
